...
export async function createViemAdapterFromWallet(passedProvider?: any) {
  // Use the exact provider from Reown AppKit if passed, otherwise fall back
  const provider = passedProvider || (window as any).ethereum;
