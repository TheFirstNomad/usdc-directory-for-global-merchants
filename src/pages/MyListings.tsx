...
const MyListings = () => {
  ...
  useEffect(() => {
    ...
    supabase
      .from("partners")
      .select("*")
      .eq("wallet_address" as string, address.toLowerCase())
      .order("created_at", { ascending: false })
      .then(({ data, error }) => {
        if (!error && data) {
          setListings(data as unknown as Partner[]);
        }
        setLoading(false);
      });
  }, [isConnected, address]);
      ...
      <section className="bg-gradient-to-b from-primary/5 to-background py-14 px-6">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-muted-foreground text-base">
            Manage your business listings on USDC Directory
          </p>
        </div>
      </section>
      ...
      <main className="flex-1 max-w-3xl mx-auto w-full px-6 py-10">
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-muted-foreground mb-6">
              You haven't listed any businesses. Get started by listing your first business!
            </p>
            <Link to="/submit">
          ...
          <div className="space-y-4">
            {listings.map((listing) => (
              <div
                key={listing.id}
                className="bg-card border border-border rounded-2xl p-6 flex items-center justify-between gap-4"
              >
