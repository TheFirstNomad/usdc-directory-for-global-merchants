/**
 * submit-free-listing — Creates a pending (unpaid) listing for admin review.
 *
 * POST body:
 *   { wallet_address, data: { company_name, description, website?, categories[], region?, logo_url?, contact_email? } }
 *
 * Inserts into partners with payment_status='pending_review' (hidden from
 * partners_public view) and a submission row with status='pending'.
 * Rate-limited: max 1 pending_review per wallet at a time, max 3 total per 24h.
 */

import { createClient } from "https://esm.sh/@supabase/supabase-js@2";

const corsHeaders = {
  "Access-Control-Allow-Origin": "*",
  "Access-Control-Allow-Headers":
    "authorization, x-client-info, apikey, content-type",
};

const ALLOWED_CATEGORIES = new Set([
  "AI & Agentic Platforms","Bridge Apps","Bridge SDKs","DeFi Apps","Digital Wallets",
  "Due Diligence & Advisory","Ecommerce","Exchanges","Fintechs","Gaming",
  "Infrastructure Providers","Market Makers","Marketplaces","Neobanks","OTC Desks",
  "Payments","PR & Communications","Remittances","Security","AI Agents",
]);
const ALLOWED_REGIONS = new Set([
  "Global","Africa","Europe","Asia","North America","South America","Other",
]);

const WALLET_RE = /^0x[0-9a-fA-F]{40}$/;
const EMAIL_RE = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

function isValidUrl(s: string): boolean {
  try {
    const u = new URL(s);
    return u.protocol === "http:" || u.protocol === "https:";
  } catch {
    return false;
  }
}

function validate(body: any): { ok: true; out: any } | { ok: false; error: string } {
  if (!body || typeof body !== "object") return { ok: false, error: "Invalid body" };
  const { wallet_address, data } = body;

  if (typeof wallet_address !== "string" || !WALLET_RE.test(wallet_address)) {
    return { ok: false, error: "Invalid wallet_address" };
  }
  if (!data || typeof data !== "object") return { ok: false, error: "Missing data" };

  const company_name = String(data.company_name ?? "").trim();
  const description = String(data.description ?? "").trim();
  if (company_name.length < 1 || company_name.length > 80) return { ok: false, error: "company_name 1-80 chars" };
  if (description.length < 1 || description.length > 500) return { ok: false, error: "description 1-500 chars" };

  let website: string | null = null;
  if (data.website) {
    const w = String(data.website).trim();
    if (w.length > 200 || !isValidUrl(w)) return { ok: false, error: "Invalid website URL" };
    website = w;
  }

  let contact_email: string | null = null;
  if (data.contact_email) {
    const e = String(data.contact_email).trim();
    if (e.length > 255 || !EMAIL_RE.test(e)) return { ok: false, error: "Invalid contact_email" };
    contact_email = e;
  }

  const categories: string[] = Array.isArray(data.categories) ? data.categories.map(String) : [];
  if (categories.length === 0) return { ok: false, error: "Select at least one category" };
  if (categories.length > 5) return { ok: false, error: "Max 5 categories" };
  for (const c of categories) {
    if (!ALLOWED_CATEGORIES.has(c)) return { ok: false, error: `Invalid category: ${c}` };
  }

  const region = data.region ? String(data.region) : "Global";
  if (!ALLOWED_REGIONS.has(region)) return { ok: false, error: `Invalid region: ${region}` };

  let logo_url: string | null = null;
  if (data.logo_url) {
    const l = String(data.logo_url).trim();
    if (l.length > 500 || !isValidUrl(l)) return { ok: false, error: "Invalid logo_url" };
    logo_url = l;
  }

  return {
    ok: true,
    out: { wallet_address, company_name, description, website, contact_email, categories, region, logo_url },
  };
}

Deno.serve(async (req) => {
  if (req.method === "OPTIONS") {
    return new Response("ok", { headers: corsHeaders });
  }
  if (req.method !== "POST") {
    return new Response(JSON.stringify({ error: "Method not allowed" }), {
      status: 405,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  }

  try {
    const supabase = createClient(
      Deno.env.get("SUPABASE_URL")!,
      Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!,
    );

    const body = await req.json();
    const v = validate(body);
    if (!v.ok) {
      return new Response(JSON.stringify({ error: v.error }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    const { wallet_address, company_name, description, website, contact_email, categories, region, logo_url } = v.out;
    const walletLower = wallet_address.toLowerCase();

    // Rate limit 1: no existing pending_review for this wallet
    const { data: existingPending } = await supabase
      .from("partners")
      .select("id")
      .eq("wallet_address", walletLower)
      .eq("payment_status", "pending_review")
      .maybeSingle();

    if (existingPending) {
      return new Response(
        JSON.stringify({ error: "You already have a listing pending review. Please wait for admin approval before submitting another." }),
        { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    // Rate limit 2: max 3 free submissions per wallet per 24h
    const since = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
    const { count } = await supabase
      .from("partners")
      .select("id", { count: "exact", head: true })
      .eq("wallet_address", walletLower)
      .eq("payment_status", "pending_review")
      .gte("created_at", since);

    if ((count ?? 0) >= 3) {
      return new Response(
        JSON.stringify({ error: "Daily free submission limit reached. Try again tomorrow or use paid listing." }),
        { status: 429, headers: { ...corsHeaders, "Content-Type": "application/json" } },
      );
    }

    // Insert pending partner
    const { data: newPartner, error: partnerErr } = await supabase
      .from("partners")
      .insert({
        name: company_name,
        description,
        website,
        categories,
        region,
        logo_url,
        wallet_address: walletLower,
        payment_status: "pending_review",
        featured: false,
      })
      .select("id")
      .single();

    if (partnerErr) {
      console.error("[submit-free-listing] partner insert error:", partnerErr);
      throw partnerErr;
    }

    // Mirror submission record for the admin queue
    const { error: subErr } = await supabase.from("submissions").insert({
      company_name,
      contact_email: contact_email || "not-provided@usdc.directory",
      website: website || "https://usdc.directory",
      description,
      categories,
      region,
      logo_url,
      wallet_address: walletLower,
      payment_id: null,
      payment_status: "free",
      status: "pending",
      partner_id: newPartner.id,
    });
    if (subErr) console.error("[submit-free-listing] submission insert error:", subErr);

    return new Response(
      JSON.stringify({ success: true, partner_id: newPartner.id, status: "pending_review" }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  } catch (err) {
    console.error("[submit-free-listing] error:", err);
    return new Response(
      JSON.stringify({ error: "Internal server error" }),
      { status: 500, headers: { ...corsHeaders, "Content-Type": "application/json" } },
    );
  }
});
